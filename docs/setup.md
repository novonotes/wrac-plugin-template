...
サブモジュールを含めてクローンし、git 履歴を切り離して新しいリポジトリとして初期化します。

```sh
git clone --recurse-submodules https://github.com/novonotes/wxp-gain-example.git /path/to/my_plugin
...
rg "wxp_example_gain_plugin|WXP Example Gain|com\.novo-notes\.wxp-example-gain|wxp-example-gain-plugin" \
    --glob '!node_modules' --glob '!dist' --glob '!*.lock' --glob '!package-lock.json' --glob '!*.zip'
```

...
use wrac_xtask::{
    ...
    WracCommand, WracInstallScope, WracUninstallScope, WracWorkspace, XtaskConfig,
};
...
fn main() -> wrac_xtask::Result<()> {
    ...
    workspace.run(Cli::parse().command.into())
}
...
enum Commands {
    #[command(about = "Build plugin and standalone artifacts.")]
    Build(BuildArgs),
    #[command(about = "Build and install plugin artifacts.")]
    Install(InstallArgs),
    #[command(about = "Remove installed plugin artifacts.")]
    Uninstall(UninstallArgs),
    #[command(about = "Build and validate plugin artifacts.")]

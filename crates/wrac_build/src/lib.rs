...
fn write_plugin_products(
    ...
    let uses_aax = metadata.supports_format(wrac_manifest::PluginFormat::Aax);
    let mut rust =
    ...
    if uses_aax {
        let aax_manufacturer_id = metadata.aax_manufacturer_id.as_ref().ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidData, "missing AAX manufacturer ID")
        })?;
        rust.push_str(&format!(
    ...
    for (index, plugin) in metadata.plugins.iter().enumerate() {
        ...
        if uses_aax {
            validate_plugin_aax_metadata(plugin)?;
    ...
    for (index, plugin) in metadata.plugins.iter().enumerate() {
        ...
        rust.push_str("        }),\n");
        if uses_aax {
            let aax_metadata = plugin_aax_metadata(plugin)?;
            ...
            rust.push_str(&format!(
                "            categories: {},\n",
                aax_categories_literal(aax_metadata.categories)?
            ));
            rust.push_str("            manufacturer_id: AAX_MANUFACTURER_ID,\n");
            rust.push_str(&format!(
                "            product_id: {},\n",
                fourcc_literal(aax_metadata.product_id)?
            ));
            rust.push_str(&format!(
...
struct PluginAaxMetadata<'a> {
    categories: &'a [String],
    product_id: &'a str,
}

fn plugin_aax_metadata(plugin: &wrac_manifest::PluginProduct) -> io::Result<PluginAaxMetadata<'_>> {
    let categories = plugin.aax_categories.as_deref().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "missing AAX categories for plugin {} while formats contains aax",
                plugin.plugin_id
            ),
        )
    })?;
    ...
    let product_id = plugin.aax_product_id.as_deref().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "missing AAX product ID for plugin {} while formats contains aax",
                plugin.plugin_id
            ),
        )
    })?;
    if plugin.aax_stem_configs.is_empty() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "missing AAX stem configs for plugin {} while formats contains aax",
                plugin.plugin_id
            ),
        ));
    }
    Ok(PluginAaxMetadata {
        categories,
        product_id,
    })
}

fn validate_plugin_aax_metadata(plugin: &wrac_manifest::PluginProduct) -> io::Result<()> {
    plugin_aax_metadata(plugin).map(|_| ())
}
...
mod tests {
    ...
    #[test]
    fn plugin_descriptor_codegen_skips_aax_when_format_is_not_supported() {
        let _env_lock = ENV_LOCK.lock().expect("env lock must not be poisoned");
        let temp_dir = unique_temp_dir();
        fs::create_dir_all(&temp_dir).unwrap();
        let manifest_path = temp_dir.join("wrac-plugin.toml");
        let output_path = temp_dir.join("wrac_plugin_products.rs");
        fs::write(&manifest_path, NON_AAX_FIXTURE_MANIFEST).unwrap();

        let manifest = wrac_manifest::read_dedicated_manifest(&manifest_path).unwrap();
        super::write_plugin_products(&manifest, &output_path).unwrap();
        let generated = fs::read_to_string(&output_path).unwrap();

        assert!(!generated.contains("AAX_MANUFACTURER_ID"));
        assert!(!generated.contains("AAX_PACKAGE_VERSION"));
        assert!(!generated.contains("AAX_STEM_CONFIGS"));
        assert!(generated.contains("aax: None,"));

        fs::remove_dir_all(temp_dir).unwrap();
    }

    #[test]
    fn plugin_descriptor_codegen_requires_aax_metadata_when_format_is_supported() {
        let _env_lock = ENV_LOCK.lock().expect("env lock must not be poisoned");
        let temp_dir = unique_temp_dir();
        fs::create_dir_all(&temp_dir).unwrap();
        let manifest_path = temp_dir.join("wrac-plugin.toml");
        ...
        let mut manifest = wrac_manifest::read_dedicated_manifest(&manifest_path).unwrap();
        manifest.plugins[0].aax_categories = None;

        let error = super::write_plugin_products(&manifest, &output_path).unwrap_err();
        assert_eq!(error.kind(), std::io::ErrorKind::InvalidData);
        assert!(
            error
                .to_string()
                .contains("missing AAX categories for plugin com.your-company.wrac-gain")
        );
...
formats = [
  { type = "clap", distribution = "public" },
  { type = "vst3", distribution = "public" },
  { type = "au", distribution = "public" },
  { type = "aax", distribution = "public" },
]
...
"#;
    ...
    const NON_AAX_FIXTURE_MANIFEST: &str = r#"
schema_version = 1

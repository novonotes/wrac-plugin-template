...
fn write_plugin_products(
    ...
    let uses_aax = metadata
        .supported_formats
        .iter()
        .any(|format| *format == wrac_manifest::PluginFormat::Aax);
    let mut rust =
...
fn plugin_aax_metadata(plugin: &wrac_manifest::PluginProduct) -> io::Result<PluginAaxMetadata<'_>> {
    let categories = plugin.aax_categories.as_deref().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "missing AAX categories for plugin {} while supported_formats contains aax",
                plugin.plugin_id
            ),
        )
    })?;
    ...
    let product_id = plugin.aax_product_id.as_deref().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "missing AAX product ID for plugin {} while supported_formats contains aax",
                plugin.plugin_id
            ),
        )
    })?;
    if plugin.aax_stem_configs.is_empty() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "missing AAX stem configs for plugin {} while supported_formats contains aax",
                plugin.plugin_id
            ),
        ));
    }
